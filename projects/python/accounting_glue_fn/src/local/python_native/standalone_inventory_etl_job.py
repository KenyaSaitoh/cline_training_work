# Inventory ETL Job - Python Native Version (ThreadPoolExecutor)
#
...
def process_record(record, transformer):
    # 単一レコードを処理（在庫移動1件 -> 会計仕訳2件）
    try:
        transformed_list = transformer.transform_record(record)
        return ('success', transformed_list)
    except Exception as e:
        return ('error', {'record': record, 'error': str(e)})


def main():
    parser = argparse.ArgumentParser(description='Inventory ETL Job (Python Native)')
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info("Inventory ETL Job Started (Python Native - ThreadPoolExecutor)")
        logger.info("=" * 60)
        logger.info(f"Batch ID: {batch_id}")
        logger.info(f"Input File: {input_path}")
        logger.info(f"Output File: {output_path}")
        logger.info(f"Movement Types: {args.movement_types}")
        ...
        with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
            # 全レコードを並列処理
            futures = {
                executor.submit(process_record, record, transformer): record
            ...
            for future in as_completed(futures):
                ...
                if status == 'success':
                    # Inventoryは1レコード -> 2レコード（借方・貸方）に変換
        ...
        if error_records:
            ...
            for err in error_records[:5]:  # 最初の5件のみ表示
                logger.error(f"  Movement ID: {err['record'].get('movement_id')}, Error: {err.get('error')}")
